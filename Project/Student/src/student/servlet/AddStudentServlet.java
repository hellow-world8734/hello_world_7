...
public class AddStudentServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (i == 1) {
				request.getSession().setAttribute("message", "<em style='color: green;'>添加学生"+student.getName()+"成功！</em>");
			} else {
				request.getSession().setAttribute("message", "<em style='color: red;'>添加学生"+student.getName()+"失败！信息冲突或不存在</em>");
			}
