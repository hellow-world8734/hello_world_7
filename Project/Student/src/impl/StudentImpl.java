...
public class StudentImpl implements IStudent {
	...
	public int excelAdd(Student student) {
		int i = 0;
		try {
			conn = DB.getConn();
			pst = conn
					.prepareStatement("INSERT INTO student (ope_id,stu_no,stu_name,stu_sex,stu_birth,cla_id) VALUES (?,?,?,?,?,?)");
			pst.setInt(1, student.getOperator().getId());
			pst.setString(2, student.getNo());
			pst.setString(3, student.getName());
			pst.setString(4, student.getSex());
			pst.setString(5, student.getBirth());
			pst.setInt(6, student.getClasses().getId());
			i = pst.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
