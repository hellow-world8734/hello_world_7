...
public class PictureImpl implements IPicture
{
	...
	// 上传新照片
	public void upload(File file, Student student, HttpServletRequest request)
	{
		try
		{
			String servletPath = request.getServletContext().getRealPath("/").toString();
			String uploadPath = servletPath + "upload";
			java.io.File uploadFile = new java.io.File(uploadPath);
			if (!uploadFile.exists()){
				uploadFile.mkdirs();
			}
			String fileName = student.getNo() + "." + file.getFileExt().toLowerCase();
			String picPath = "upload/" + fileName;
			student.setPic("../" + picPath);
			file.saveAs(picPath);
			System.out.println("头像地址："+uploadFile.getPath()+"\\"+fileName);
		} catch (Exception e)
		{
			e.printStackTrace();
		}
